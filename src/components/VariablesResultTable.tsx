...
import { Hash, ArrowRight, Download } from 'lucide-react';
        ...
        <div className="bg-white dark:bg-bg-card rounded-xl border border-slate-200 dark:border-border-subtle h-full flex flex-col shadow-sm overflow-hidden transition-colors">
            ...
            <div className="px-6 py-4 border-b border-slate-100 dark:border-border-subtle bg-slate-50/50 dark:bg-bg-dark/50 shrink-0">
                <div className="flex justify-between items-start">
                    <div>
                     ...
                     <div className="flex items-center gap-6">
                        <button
                            onClick={() => exportVariablesToExcel(uniforms, transformed, distributionName)}
                        ...
                        <div className="text-right">
                            ...
                            <p className="text-xs text-slate-400 font-black uppercase tracking-widest">Muestras</p>
                        </div>
                    </div>
                </div>
                ...
                <div className="grid grid-cols-5 gap-2 mt-4">
                    <StatCard label="Media (μ)" value={stats.mean} color="text-violet-600 dark:text-violet-400" />
                    <StatCard label="Desv. Est. (σ)" value={stats.std} color="text-violet-600 dark:text-violet-400" />
                    <StatCard label="Mínimo" value={stats.min} color="text-sky-600 dark:text-sky-400" />
                    <StatCard label="Mediana" value={stats.median} color="text-violet-600 dark:text-violet-400" />
                ...
                <table className="w-full text-sm border-collapse">
                    <thead className="sticky top-0 z-10">
                        <tr className="bg-slate-100 dark:bg-slate-800 border-b border-slate-200 dark:border-border-subtle">
                            <th className="px-4 py-2.5 text-left font-black text-xs uppercase tracking-widest text-slate-400 w-14">
                                <Hash size={14} className="inline mr-1" />#
                            </th>
                            <th className="px-4 py-2.5 text-left font-black text-xs uppercase tracking-widest text-slate-400">
                                Uniforme U(0,1)
                            </th>
                            <th className="px-4 py-2.5 text-center font-black text-xs uppercase tracking-widest text-slate-300 w-8">
                                <ArrowRight size={14} className="mx-auto" />
                            </th>
                            <th className="px-4 py-2.5 text-left font-black text-xs uppercase tracking-widest text-violet-600 dark:text-violet-400">
                                X ~ {distributionName}
                            </th>
                            <th className="px-4 py-2.5 text-left font-black text-xs uppercase tracking-widest text-slate-300 w-36">
                                Magnitud relativa
                            </th>
                        </tr>
                    </thead>
                    <tbody>
                        {transformed.map((x, i) => {
                            ...
                            return (
                                ...
                                >
                                    <td className="px-4 py-3 text-xs font-bold text-slate-300 dark:text-slate-600 tabular-nums">
                                        {i + 1}
                                    </td>
                                    ...
                                    <td className="px-4 py-2 font-mono font-bold tabular-nums">
                                        <span className={isAboveMean ? 'text-green-600 dark:text-green-400' : 'text-slate-600 dark:text-slate-300'}>
                                            {x.toFixed(6)}
                                        </span>
                                    </td>
                                    <td className="px-4 py-2">
                                        <div className="flex items-center gap-2">
                                            <div className="flex-1 h-1.5 bg-slate-100 dark:bg-slate-800 rounded-full overflow-hidden">
                                                <div
                                                    className={`h-full rounded-full transition-all ${isAboveMean ? 'bg-green-400' : 'bg-slate-300 dark:bg-slate-600'}`}
